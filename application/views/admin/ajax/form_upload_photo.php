	...
	<div class="site_box_info">
		...
		Format diperbolehkan: <b><?php
			if (isset($allowedExts)) {
				echo implode(", ", $allowedExts);
			} else echo "-unknown-";
		?></b>
	</div>
	<form method="POST" action='<?php echo base_url('/admin/media/upload'); ?>' enctype="multipart/form-data">
		<div>Unggah foto baru ke album <b><?php echo htmlspecialchars($albumName); ?></b></div>
		...
		<input type="hidden" name="act" value="photo.upload" />
	</form>
</div>
...
<div style="text-align: center; display: none;" id="form_media_uploadmore">
	<a href="#" onclick="return reshowForm();"><i class="site_icon-upload"></i> Unggah Foto Lagi</a>
</div>